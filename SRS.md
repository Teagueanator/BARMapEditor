...
| **STATUS UPDATE 2026-05-20 (Sprint 18 / D7 + C7):** F9 and F10 ship. Every F1–F11 MVP item except F12 (Launch in BAR — deferred to Sprint 32) is now feature-complete. The user can iterate a project's lighting / atmosphere / water / terrain types / etc. through a form, and the auto-generated minimap reflects whatever the layer stack composites. ||||
| **F10 minimap auto-generation** — `barme_pipeline::minimap::render_minimap` ships as a CPU bake. Diffuse base = `LayerStack::bake_diffuse` (or the height-keyed biome ramp fallback when the stack is empty), box-filter-downsampled to 1024². Lambert hill shade per-pixel keyed on `mapinfo.lighting.sun_dir` (so an authored sun direction shows up immediately in the minimap). Total time ~50–200 ms in release for an 8 SMU project. `Project.minimap_override: Option<PathBuf>` (`SCHEMA_V` 1 → 2, no-op migration) routes around the bake when the user supplies a hand-authored PNG; `copy_minimap_override` strictly validates 1024² and errors with `MinimapError::OverrideWrongDim { actual, expected }` otherwise. `build_sd7` takes an `Option<MinimapInputs>` bundling the in-memory `Heightmap` + `SlotResolver` + project path; `pymapconv::CompileInputs` carries the optional `minimap_png` through to PyMapConv's `-p` flag (flag verified at vendoring time, `devlog/stage-0-validation/logs/2026-05-17T16-57-48__pymapconv-vendoring.md`). Headless wgpu was scoped out (kickoff devlog rationale: the CPU input matches what the `.sd7` ships byte-for-byte, so a GPU render would only invite drift — plus skipping it sidesteps the cross-platform adapter-selection surface area). ||||
| **F9 mapinfo form editor** — new top-bar `Icon::MapInfo` button opens a non-modal `egui::Window` with 12 tabs (General / Map / SMF / Lighting / Atmosphere / Water / Resources / Splats / Terrain types / Custom / Raw Lua / Minimap). Form lives at `crates/barme-app/src/ui/inspector_mapinfo.rs` (~1500 LoC including tests). Edits commit on widget release (`response.lost_focus() || response.drag_stopped()`) and dispatch through `ProjectDiff::EditMapInfo { from, to }` for Ctrl-Z. New `MapInfoPatch` enum in `barme-core::mapinfo_schema` covers 49 leaf fields; `App::apply_mapinfo_patch` routes each variant to either a first-class App shadow (gravity / void_water / minimap_override / lava_atmosphere / extractor_radius / min+max_height) or to `Project.mapinfo_overrides` as a dotted-Lua-path bag for fields without a shadow yet (Sprint 27's inspector consistency refactor will promote the most-edited atmosphere / lighting subset). Tab-strip lint-dot rendering is wired against a stub `FormCtx::lint_per_tab: [u32; 12]` so Sprint 21 / C8's linter populates the array without touching UI. ||||
| **Form tooltip coverage** — every interactive widget (DragValue / ComboBox / color button / text input) carries `.on_hover_text(...)` with units / range / `mapinfo.lua` consequence. Sprint 19 harvests these into a unified catalogue. ||||
| **Water tab is read-only** in Sprint 18 — the dedicated `Tool::Water` Inspector remains the canonical entry point for the discoverable preset / behaviour / appearance flow Sprint 14 ships. The F9 Water tab is a power-user dump of all 30 WaterBlock fields; Sprint 26 (water polish) promotes it to per-field DragValues on the same `Project.water_overrides` overlay. Splats tab is read-only too (Sprint 17 retired the editable inspector). Resources tab uses text inputs only; file pickers are deferred. ||||
| **Project schema bump** — `Project::SCHEMA_V` 1 → 2 with a no-op v1→v2 migration step in `Project::run_migrations` recording the new `minimap_override` field. `Project::resolve_minimap_override(project_path) -> Option<PathBuf>` mirrors the heightmap resolution rule. ||||
| Tests: +14 net across the workspace (barme-core 264 → 267: minimap_override round-trip / v1→v2 migration / resolve-against-project-dir + 3 mapinfo_patch round-trips; barme-pipeline 117 → 124: render_minimap default-project / override passthrough / wrong-dim error / rectangular-non-1024 error / stage_minimap routing / box-filter downsample averages / hill-shade sun intensity; barme-app 240 → 244: F9 form exhaustiveness pin + sky_axis_angle round-trip + label coverage + default-project schema invariants). Workspace total 621 → 635. cargo fmt / clippy / test all green. ||||
| **Out of scope (Sprint 18)** — no headless wgpu device for the minimap (kickoff devlog rationale: CPU input matches `.sd7` byte-for-byte), no editable Water tab (Sprint 26), no first-class typed shadows for atmosphere / lighting subset (Sprint 27 — fields round-trip via `mapinfo_overrides` bag for now), no file picker for resources textures (text inputs only), no minimap preview texture caching (Sprint 19 polish — the tab shows "(preview pending)" placeholder), no Sprint 21 lint output (the form's tab-strip dots are wired against a `[0; 12]` stub), no Sprint 17 follow-ups (16-SMU OOM, orphan-texture GC, legacy `SplatConfig`-on-disk retirement — Sprint 23). ||||
| **STATUS UPDATE 2026-05-20 (Sprint 19 / U1):** Discoverability + feedback pass. NEW `crates/barme-app/src/ui/help_text.rs` ships a centralised `HelpId` enum (107 variants) + `pub fn help(id) -> &'static str` with five exhaustiveness / shortcut-validity tests. Every interactive widget across the 9 Inspector tools (`header / select / sculpt / metal / geo / feature / paint_layer / water / start_positions / procgen`) now carries `.on_hover_text(help(HelpId::Foo))`. Coverage grep `on_hover_text` jumps from 56 → 202 (sprint exit target was >200). NEW `App::inspector_sticky_chips` prepends a Symmetry + Map-size chip row to every Inspector body. NEW `widgets::section_with_hover` lets a section title carry semantic context without churning the 58 existing `section()` call sites. ||||
| **Validation chip is clickable + lint panel stub.** Top-bar validation chip + status-strip issue-count both open `crates/barme-app/src/ui/lint_panel.rs` — minimal egui::Window rendering `App::validation_summary` as a single row (Sprint 21 / C8 lands the full `LintRule` registry). Status strip drops the hard-coded `"0 issues"` for a live count. NEW top-bar `Icon::Help` button to the right of the validation chip opens the cheat-sheet modal (also reachable via `?`). Save button cites Ctrl+S in its tooltip; chord wired in `handle_keyboard` (`Ctrl+S` = Save, `Ctrl+Shift+S` = Save as), promoting `cheat_sheet::PROJECT_BINDINGS` entries from "reserved" to live. `trace!` on `lint_panel` open / close transitions. ||||
| **False-promise cleanup + brush ring + minimap symmetry.** `ui::viewport_chrome` toolbar drops the bogus `(G)` / `(L)` / `(W)` chord hints — those letters bind to tool accelerators (Procgen / PaintLayer / Water) in `handle_keyboard`, not to viewport toggles. `ui::overlay::brush_ring_color` extended from 3 sculpt brushes to 7 — mask-reveal / mask-hide / mask-smooth / mask-fill pull from `Tokens::DARK` (`t.green / t.red / t.accent / t.amber`) so they match the inspector_paint_layer brush card palette. Minimap symmetry guide rewrite: NEW `paint_minimap_symmetry` + unit-testable `minimap_symmetry_segments` derive lines from `Project.symmetry` (Horizontal / Vertical / Quad / Diagonal-Main / Diagonal-Anti / Rotational-N spokes). Original mockup-era vertical-bisector code preserved as a `// History:` annotation. ||||
| Tests: +10 net across barme-app (5 help_text exhaustiveness / chord-binding + 4 lint_panel issue_count / egui smoke + 8 minimap symmetry segment geometry + 2 overlay mask-brush colour distinctness — minus 9 dropped from a pre-Sprint 18 cleanup pass that landed alongside; net workspace count 635 → 654). cargo fmt / clippy --workspace --all-targets -- -D warnings / test --workspace all green. 5 commits on `main`. ||||
| **Out of scope (Sprint 19)** — no per-rule lint registry (Sprint 21 / C8 populates `LintPanel`), no onboarding tour (Sprint 22 reuses the `help_text` catalogue), no async build pipeline (Sprint 20 / U3), no toast queue or confirmation modals (Sprint 31 / U4 — the single `last_error` line stays today), no inspector layout refactor (Sprint 27 / U5 — Sprint 19 is annotative; widgets stay put), no help-text harvest from the Sprint 18 F9 form (the inline strings stay inline; Sprint 22 dedupes when the help center lands), no `Ctrl+Shift+H` hover-popover-mode toggle (Sprint 22). ||||
| **STATUS UPDATE 2026-05-20 (Sprint 20 / U3 — async build + log + recent projects):** Build pipeline moves off the UI thread. The synchronous `launcher::build_and_install` (10–60 s freeze on a 16-SMU map) becomes a worker-thread `BuildPlan::execute` driven from `barme-app::build_runner`. The editor stays responsive while PyMapConv runs; a centred progress overlay (`ui::build_overlay`) tracks the active `BuildStage` with sub-progress, elapsed MM:SS, and a Cancel button that flips an `Arc<AtomicBool>` polled by every subprocess wrapper. ||||
| **`BuildPlan` + `BuildStage`** (in `crates/barme-pipeline/src/build.rs`) — the legacy `build_sd7` becomes a thin wrapper around `execute_stages` with a no-op event sink and a `NEVER_CANCEL` sentinel. Stages: `RenderDiffuse` (worker), `PrepareStaging`, `RenderMinimap`, `InvokePyMapConv`, `StageSplatAssets`, `EmitMapInfoLua`, `EmitMetalLayoutLua`, `EmitStartboxesLua`, `EmitFeaturePlacerLua`, `PackageSd7`, `InstallToBar` (worker), `Done`. `cumulative_fraction` per stage drives the overlay progress bar (monotone non-decreasing — pinned by `cumulative_fractions_are_monotone`). ||||
| **`invoke_with_streaming`** (in `build.rs`) — one shared primitive for all subprocess invocations. Pipes stdout + stderr through a pair of reader threads that fire a per-line callback (`Fn(&str, LogStream) + Sync`); `cancel.load(Relaxed)` is polled at 50 ms while waiting on the child and triggers `child.kill()` when set. Returns `(ExitStatus, captured_stdout, captured_stderr)` so PyMapConv's exit-1-on-success quirk normalisation (artifact-presence over exit code) keeps working post-stream. DNTS Compressonator goes through the same path with a `tracing::trace!` callback. ||||
| **Worker thread orchestration** (`crates/barme-app/src/build_runner.rs`) — `BuildState` enum on App (`Idle`/`Running`/`Done`/`Failed`/`Cancelled`); each non-Idle variant owns its `Arc<Mutex<VecDeque<BuildLogLine>>>` ring buffer (cap 5 000 lines, PITFALL #1). `App::build_and_install` snapshots project + heightmap, builds an `OwnedSlotResolver` (clones the slot registry — ~2 KB), packages everything into `WorkerInputs`, and spawns the worker. `App::poll_build_state` drains up to 100 events per frame from an `mpsc::Receiver<BuildEvent>` (PITFALL #4 — bounded drain keeps frame budget honest). ||||
| **Build log panel** (`ui::build_log`) — floating `egui::Window` with stream-tinted monospace lines (stdout default, stderr yellow, info muted, error red), auto-scroll to bottom. Footer ships `[Copy]` (`ctx.copy_text`), `[Save log…]` (rfd file picker → `apply_build_log_clicks` writes the buffer), `[Clear]` (truncates in-place). Auto-opens on `BuildState::Failed`. Reachable from the progress overlay's "View log…" button, the status-strip click, and `File > Build > Show log…`. ||||
| **Status strip live updates** (`status_strip` in `main.rs`) — the install slot becomes a live chip that tracks `BuildState`: Running ("Building: <stage> · Ns" in blue), Done ("✓ name.sd7 in 14s" in green), Failed ("✗ Build failed: …" in red), Cancelled (muted). Every non-Idle variant is click-to-show-log; Idle with a prior install keeps the green install chip from before. ||||
| **Recent projects** — `EditorConfig.recent_projects: VecDeque<PathBuf>` (cap 10) added with `#[serde(default)]` forward-compat. `App::save_to` + `App::open_from` push on success; missing files at open silently drop from the list (PITFALL #8 — no per-miss dialog; the single `last_error` line communicates). `File > Recent projects (N)` submenu lists entries with filename + full-path hover, plus a `Clear recent projects` row. Empty-state CTA in `viewport_chrome.rs` gains a "Recent projects" block of up to 5 clickable rows (filename + parent dir, hover shows full path). New `FileAction::OpenPath(PathBuf)` skips the file dialog. ||||
| **Save-before-build guard** (`ui::save_before_build`) — stub `egui::Window` modal with three buttons (Save & build / Build without saving / Cancel) that opens when the user requests a build with `App.dirty = true` and a saved project path. New-project Save & build falls back to a `pick_save_path` dialog. Sprint 31 / U4 promotes the stub to a proper modal primitive (backdrop + escape key); the API is intentionally minimal so the swap is mechanical. ||||
| **Disk-space sanity** — `build_runner::run_worker_build`'s `RenderDiffuse` stage probes the temp dir's filesystem via `fs2::available_space`. <2 GB free emits a `Warn`-stream log line that surfaces in the build log panel; the build proceeds (PITFALL — non-gating, the user might know better than the heuristic). `fs2` added to workspace deps + barme-app's manifest. ||||
| **Cleanups + tests** — legacy synchronous `launcher::build_and_install` and its private `synth_biome_bmp`/`biome_ramp` helpers retired; their sole caller (the App) now snapshots + spawns. `LauncherError::TextureSynth` variant removed. New tests: `invoke_with_streaming_collects_lines_in_order`, `invoke_with_streaming_cancels_long_running_child` (sleep 30 returned in <2 s after flag set), `cumulative_fractions_are_monotone`, `check_cancel_respects_flag`, `closure_sink_collects_events_in_order`, plus 4 `OwnedSlotResolver` / `ring_buffer_caps_at_5000` / `drain_events_updates_state_within_budget` in `build_runner::tests`, 2 `apply_click` tests on the progress overlay, 2 `render_log_as_text` tests on the build log panel, 6 recent-projects round-trip / push / dedupe / cap / remove / clear tests in `config::tests`, and 2 `save_before_build` modal smoke tests. ~16 net new tests; +1 (`fs2 = 0.4`) dep. cargo fmt / clippy --workspace --all-targets -- -D warnings / test --workspace all green. ||||
| **Out of scope (Sprint 20)** — no async procgen apply (Sprint 24 / T2 — multithreading is its own arc), no Launch in BAR (Sprint 32 / F12 builds on this sprint's worker-thread pattern), no proper toast queue or modal primitives (Sprint 31 / U4 — the save-before-build modal stays a stub), no build log persistence across editor sessions (in-memory only; `Save log…` is the escape hatch), no multi-build queue (one build at a time), no autosave / 60-s checkpointing (Sprint 32 / NFR-Crash-safety), no process-group kill for PyMapConv's Compressonator grandchildren (best-effort `Child::kill` on the direct child only; orphans die in ~5 s on their own). ||||
| **STATUS UPDATE 2026-05-20 (Sprint 21 / C8 — Lint My Map):** C8 ships. The `App::validation_summary` heuristics retire in favour of a real `LintRule` registry in `barme-pipeline::lint`. ||||
| **`LintRule` registry** — 31 variants covering every silent failure mode in PITFALLS.md §1–§28 plus the source-audit additions (FINDINGS §12 / NEW-1..NEW-10). 10 hard-error rules (Build button gated), 18 warnings (surface but non-blocking), 3 info-tier convention notes. Every rule has positive + negative test coverage: 81 tests in `barme-pipeline::lint`. `lint()` walks every check sub-millisecond on a 32-SMU project with hundreds of features; runs every frame against `App::lint_summary` (Sprint 20 prequel: "no debounce needed for Sprint 21"). ||||
| **Hard-error rules** — `ModtypeNotThree`, `DependMissingMapHelper`, `SmtFileNameZeroMissing`, `NameOrMapfileOrVersionMissing`, `VoidWaterWithPlaneColor`, `TeamsEmpty`, `FeatureNotInStockManifest`, `FogStartEqualsFogEnd`, `HeightmapDimsWrong`. `SplatDetailNormalTexWithoutSpecular` reclassified to Warning after the Sprint 14 / C9 `validation_summary_warns_when_slot_bound_without_specular` test pinned its prior tone (FINDINGS §7.2 — engine no longer gates DNTS on `specularTex` at the C++ render-state level, so a missing specular is a quality issue not a broken build). ||||
| **Warning rules** — the Sprint 14 / C9 WARN-tier (DNTS + min<0 LOS bug, terrain-water mismatch both directions) migrated from `validation_summary` into the registry. New: sun-dir zero-vector, deprecated `skyDir` / `minimapRotation` imports, extractor 500 (hard fix to 80), tidal without water surface, ≥4-team large-map slot count, multi-team startboxes missing, DNTS without `detailTex`, geos array import (Zero-K convention guard), custom metalmap with Lua spots, sun-dir w over-saturation (1e9 leakage guard), legacy splat normal subtable, featureplacer rot shape import, LuaGaia bootstrap opt-out, maxMetal outside `0.5..=5.0`. ||||
| **Info-tier rules** — gravity drift outside 100..=160, extractor radius drift from 80 (skipped at 500), voidGround on with default `voidAlphaMin`. ||||
| **Future-import guards** — rules that cannot fire from editor-authored state (sky_dir / minimapRotation, geos array, featureplacer quoted rot, LuaGaia opt-out, custom metalmap) key off `Project.mapinfo_overrides` so F13 / Stage 2's import path can populate the overrides bag and the lint surfaces immediately. Each has a positive + negative test that constructs a synthetic override to exercise the predicate. ||||
| **UI wiring** — Sprint 19's `lint_panel.rs` stub gets its real body: header chip with per-severity counts + "{n}/{total} rules passing" footer; grouped issue list (Errors first); each row shows the rule title, message, optional `field_path` chip, and a Fix button when the rule carries a `LintFix::MapInfoPatch`. Fix clicks return the patch which the App dispatches through `apply_mapinfo_patch` + `ProjectDiff::EditMapInfo`, so Ctrl-Z restores the prior value (matches F9 form's pattern). Sprint 18's F9 tab-strip dots populate via `lint_panel::tab_counts(&lint_summary)` which routes `field_path` prefixes to `MapInfoTab` indices. ||||
| **Build button gating** — `App::lint_error_count` aggregates `lint_summary`. `top_bar_right_block` greys the Build button when count > 0; the disabled-hover tooltip lists the top 3 errors + "Click the chip to open the lint panel." `build_and_install` itself refuses with `last_error = "Build blocked by N lint error(s)…"` and auto-opens the lint panel — the worker-side gate catches keyboard-accelerator / dirty-save funnel paths the UI gate can't see. Save / Save As / Export Raw Lua / inspector edits stay enabled. ||||
| **App glue** — new `App::lint_summary: Vec<LintIssue>` + `App::lint_stock_manifest: StockManifest` (catalog loaded once via `include_str!`d JSON). `App::recompute_lint` runs at the top of `update()` so every panel sees a consistent view. `App::validation_summary` now aggregates from the registry: errors first → Err tone + rule title, warnings → Warn tone + rule title, otherwise Ok. The App-level pre-lint gates (no-heightmap, heightmap-mismatch, procgen-expression-error) keep their priority since the linter can't see App-only state. ||||
| **Tests** — 81 in `barme-pipeline::lint` (29 rule positive + 28 rule negative + 5 metadata + multi-fire composition + wizard-style fixture assertions), 4 tab-mapper / panel smoke tests in `lint_panel::tests`, 2 build-gating tests in `barme-app::tests` (`lint_error_count_aggregates_hard_errors`, `build_and_install_refuses_with_hard_errors`), and the 4 pre-existing `validation_*` tests rerouted through `recompute_lint` + a wizard-style ally-group seed. cargo fmt / clippy --workspace --all-targets -- -D warnings / test --workspace all green. ||||
| **Stage 1 internal-work milestone:** every F-item except F12 (Launch in BAR, deferred to Sprint 32) is now feature-complete. F1–F11 + F13–F15 (math / data subsets) ship; the editor produces a `.sd7` that passes a programmatic lint covering every silent failure mode in PITFALLS.md. Sprint 22 = onboarding + contextual help + command palette (third and final UI polish sprint). ||||
| **Out of scope (Sprint 21)** — F12 Launch in BAR (Sprint 32), F13 decompile / import (Stage 2), F14 procgen v2 (Stage 2), C8's "auto-fix all warnings" bulk operation (defer; risky to apply without review), PITFALLS §26-28 lint rules (look_at_lh sign-flip, GetWaterPlaneLevel consteval, min_height shader plumbing — these are renderer-side, already enforced by the Sprint 13 / 14 codepath; not lint-surfaceable from project state alone), build-overlay's "show in lint panel" dialog (the disabled-hover tooltip + auto-open on build-gate satisfy the contract). ||||
